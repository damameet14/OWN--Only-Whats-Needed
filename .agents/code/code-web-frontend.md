...
| 2026-04-18 | Fixed `getSelectedSegment()` (`editor.js`) to return segment only when ALL words selected, preventing single-word clicks from modifying segment styles. Fixed `setWordMarker()` (`editor.js`) to re-render Segments panel after marker changes. Fixed timeline segment click (`timeline.js`) to seek to midpoint instead of start time. |
